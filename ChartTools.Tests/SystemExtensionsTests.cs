...
public class SystemExtensionsTests
{
    ...
    [TestMethod] public void ReplaceSectionNullStartReplace() => Assert.ThrowsException<NullReferenceException>(() => trueArray.ReplaceSection(new([], null!, b => true, true)).ToArray());
    [TestMethod] public void ReplaceSectionNullEndReplace() => Assert.ThrowsException<NullReferenceException>(() => trueArray.ReplaceSection(new([], b => true, null!, true)).ToArray());
